...
import { FlaskConical, Lightbulb, Settings, CheckCircle2, Atom } from 'lucide-react';
            ...
            {t.rnd.fields.map((field: any, idx: number) => (
                <ScrollReveal key={idx} delay={idx * 0.1}>
                    <div className="bg-white rounded-2xl p-8 h-full shadow-[0_8px_30px_rgba(0,0,0,0.04)] hover:shadow-[0_12px_40px_rgba(0,0,0,0.08)] border border-gray-100 transition-all duration-300 hover:-translate-y-1">
                        ...
                        <h3 className="text-xl font-bold text-gray-900 mb-6">{field.title}</h3>
                        <ul className="space-y-4">
                            {field.items.map((item: string, iIdx: number) => (
                                <li key={iIdx} className="flex items-start gap-3 text-gray-600">
                                    <CheckCircle2 className="w-5 h-5 text-brand-blue flex-shrink-0 mt-0.5" />
                                    <span className="leading-relaxed break-keep font-medium text-[15px]">{item}</span>
                                </li>
                            ))}
                        </ul>
                    </div>
