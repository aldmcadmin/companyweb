           ...
           <ScrollReveal>
             <div className="max-w-4xl mx-auto text-center">
                ...
                <p className="text-sm sm:text-base md:text-xl text-gray-500 leading-relaxed break-keep">
                  {t.quality?.intro_desc || '대우경금속은 고객에게 무결점 제품을 제공하기 위해 최첨단 측정 설비와 철저한 다중 검사 시스템을 구축하고 있습니다. 원소재 입고부터 최종 출하까지 모든 공정을 철저히 감독합니다.'}
                </p>
             </div>
           </ScrollReveal>
        </section>
        ...
        <section>
          <ScrollReveal>
             <div className="flex items-center gap-3 mb-8 md:mb-10 border-b border-gray-200 pb-4">
                ...
                <h2 className="text-2xl md:text-3xl font-bold text-gray-900 tracking-tight break-keep">{t.quality?.equip_title || '측정설비 보유현황'}</h2>
             </div>
          </ScrollReveal>
                 ...
                 <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200">
                        <th className="py-3 px-4 md:py-4 md:px-6 font-bold text-gray-700 text-xs md:text-base border-r border-gray-200 w-1/4 break-keep">{t.quality?.th_name || '설비명'}</th>
                        <th className="py-3 px-4 md:py-4 md:px-6 font-bold text-gray-700 text-xs md:text-base border-r border-gray-200 w-1/4 break-keep">{t.quality?.th_maker || '제조사 / 모델명'}</th>
                        <th className="py-3 px-4 md:py-4 md:px-6 font-bold text-gray-700 text-xs md:text-base break-keep">{t.quality?.th_purpose || '측정 목적 및 주요 기능'}</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {equipments.map((tool, idx) => {
                        ...
                        const translatedItem = qualityTranslations?.items?.[translationKey];
                ...
                <ScrollReveal key={tool.id} delay={index * 0.1}>
                  <div className="bg-white rounded-2xl overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 group hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)] transition-shadow duration-300 flex flex-col h-full">
                     ...
                     <div className="p-5 md:p-6 flex flex-col flex-grow">
                       <h3 className="text-lg md:text-xl font-bold text-gray-900 mb-2 break-keep">{translatedItem?.name || tool.name}</h3>
                       <p className="text-sm md:text-base text-gray-500 leading-relaxed flex-grow break-keep">{translatedItem?.description || tool.description}</p>
                       <div className="mt-5 md:mt-6 pt-4 border-t border-gray-100 flex items-center gap-2 text-brand-blue font-semibold text-xs md:text-sm">
                          ...
                          <span className="break-keep">{t.quality?.calibrated || '정밀 검교정 완료'}</span>
                       </div>
                     </div>
                  </div>
                </ScrollReveal>
           ...
           <ScrollReveal>
             <div className="bg-white rounded-3xl p-6 sm:p-8 md:p-12 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 flex flex-col md:flex-row items-center justify-between gap-6 md:gap-8 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-shadow">
                <div className="text-center md:text-left">
                  <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 mb-3 md:mb-4 tracking-tight break-keep">{t.quality?.cert_title || '국제적으로 입증된 품질 관리'}</h3>
                  <p className="text-sm sm:text-base md:text-lg text-gray-500 break-keep">{t.quality?.cert_desc || 'IATF 16949, ISO 9001 등 대우경금속의 품질 경영 인프라를 증명하는 인증 내역을 확인하세요.'}</p>
                </div>
